...
function TrocoCalculator({ total }) {
  const [valorRecebido, setValorRecebido] = useState("");
  const troco = valorRecebido ? Number(valorRecebido) - total : 0;
  const notasRapidas = [10, 20, 50, 100, 200];

  return (
    <div style={{ background: C.card2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14 }}>
      <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>💵 Calcular Troco</div>

      {/* Notas rápidas */}
      <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
        {notasRapidas.filter(n => n >= total || n >= total / 2).map(n => (
      ...
      <div style={{ ...base.modalBox, maxWidth: 520 }}>
        ...
        {modo === "total" && (
          <div style={{ marginBottom: 16 }}>
            ...
            {pagamento === "dinheiro" && (
              <TrocoCalculator total={totalComanda} />
            )}
          </div>
        )}
